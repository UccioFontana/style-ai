...
class _AnalyzeScreenState extends State<AnalyzeScreen> {
  ...
  List<MapEntry<String, String>> _buildRows(dynamic responseData) {
    ...
    return [
      ...
      MapEntry('RMS', _format(v(['dynamics', 'rms_dbfs']), suffix: ' dBFS')),
      MapEntry('RMS lineare', _format(v(['dynamics', 'rms_linear']))),
      MapEntry(
        'Crest Factor',
      ...
      MapEntry('Canali', _format(v(['audio_properties', 'channels']))),
      MapEntry(
        'Bit depth',
        _format(v(['audio_properties', 'bit_depth']), suffix: ' bit'),
      ),
      MapEntry('Formato', _format(v(['audio_properties', 'format']))),
      MapEntry('Subtype', _format(v(['audio_properties', 'subtype']))),
      MapEntry(
        'Clipping detection',
        v(['clipping', 'clipping_detected']) == true ? 'Sì' : 'No',
      ),
      MapEntry('Campioni clipped', _format(v(['clipping', 'clipped_samples']))),
      MapEntry(
        'Clipping %',
  ...
  Widget _buildMetricsTable() {
    return Container(
      constraints: const BoxConstraints(maxWidth: 700),
      child: Table(
        border: TableBorder.all(color: AppTheme.textSecondary.withOpacity(0.4)),
        ...
        children: [
          ...
          ..._metricsRows.map(
            (row) => TableRow(children: [_cell(row.key), _cell(row.value)]),
          ),
        ],
      ),
    );
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (_aiReviewText != null &&
                              _aiReviewText!.isNotEmpty) ...[
                            const SizedBox(height: 24),
                            _buildAiReviewBox(),
                          ],
